...
func New(partRepo repository.PartRepository) *seeder {
	return &seeder{partRepo: partRepo}
}

func (s *seeder) Seed(ctx context.Context) error {
	list, err := s.partRepo.List(ctx, model.PartsFilter{})
	if err != nil {
		return err
	}
	if len(list) > 0 {
	...
	if err := s.partRepo.Add(ctx, part1); err != nil {
		return err
	}
	...
	if err := s.partRepo.Add(ctx, part2); err != nil {
		return err
	}
